...
class ContactForm extends Component {
  ...
  sendEmail() {
    if (this.checkBeforeSend()) {
      ...
      axios({
        method: 'POST',
        url: `https://formspree.io/f/xyyojjyj`,
        data: {
  ...
  handleCanSend(event) {
    if (!this.emailValidator(this.refs.email.value))
      this.setState({ errorMessage: 'There must be a valid email here!' });
    else if (this.refs.text.value.split('').length == 0)
      this.setState({ errorMessage: 'There must be some text here!' });
    else this.setState({ errorMessage: '' });
  }

  checkBeforeSend() {
    return (
      this.refs.text.value.split('').length > 0 &&
      this.emailValidator(this.refs.email.value)
    );
  }
  ...
  render() {
    ...
    if (this.state.isSending) {
      ...
      message = (
        <h2>
          {' '}
          Hey! An error has occoured! Please try to refresh the page and send it
          again.
        </h2>
      );
    }
    ...
    return (
      <div className="contactFormWrapper">
        <div
          className={
            'message ' +
            (this.state.successfulSended === true
              ? 'show'
              : this.state.errorSending == true
              ? 'show'
